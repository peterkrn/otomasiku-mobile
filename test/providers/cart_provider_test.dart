...
class _MockCartRepository implements CartRepository {
  ...
  bool addShouldFail;
  bool addReplacesExistingProductQuantity;
  ...
  _MockCartRepository({
    ...
    this.addReplacesExistingProductQuantity = false,
  ...
  Future<CartItem> addItem({
    ...
    if (addReplacesExistingProductQuantity) {
      final existingIndex = serverItems.indexWhere(
        (item) => item.productId == productId,
      );
      if (existingIndex >= 0) {
        final existing = serverItems[existingIndex];
        ...
        serverItems[existingIndex] = updated;
        return updated;
      }
    }
    final newItem = _item(
...
void main() {
  ...
  group('Bug #1 — selectedCartItemsProvider populated for buy-now', () {
    test(
      'selectedCartItemsListProvider returns item when cartItemId is selected',
      () async {
        final repo = _MockCartRepository(
          serverItems: [_item(productId: 'prod-1')],
        );
    ...
    test(
      'selectedCartItemsListProvider is empty when selectedCartItemsProvider is empty',
      () async {
        final repo = _MockCartRepository(
          serverItems: [_item(productId: 'prod-1')],
        );
  ...
  group('Bug #2 — loadCart does not overwrite optimistic state', () {
    ...
    test(
      'guard: loadCart is skipped when items already exist (cart_screen guard)',
      () async {
        ...
        expect(
          shouldLoad,
          isFalse,
          reason: 'Guard must prevent loadCart when items exist',
        );
      },
    );

    test(
      'late loadCart response does not overwrite newer optimistic add state',
      () async {
    ...
    test(
      'badge count (totalItems) matches actual item count after add',
      () async {
        final repo = _MockCartRepository();
        final container = _container(repo);
        addTearDown(container.dispose);
  ...
  group('Bug #3 — removeItem cleans up selectedCartItemsProvider', () {
    test(
      'after removeItem, cartItemId is no longer in selectedCartItemsProvider',
      () async {
        final repo = _MockCartRepository(
          serverItems: [
        ...
        container.read(selectedCartItemsProvider.notifier).state = current
            .where((id) => id != 'item-1')
            .toSet();
  ...
  group('Bug #10 — clearCart empties state immediately', () {
    test(
      'clearCart empties items optimistically before API responds',
      () async {
        final repo = _MockCartRepository(
          serverItems: [
            _item(),
  ...
  group('Launch plan — partial checkout preserves unselected items', () {
    test('removeItemsLocally removes only submitted cart item ids', () async {
      ...
      container.read(cartProvider.notifier).removeItemsLocally({
        'item-1',
        'item-3',
      });
  ...
  group('CartNotifier — general', () {
    ...
    test(
      'addItem updates persisted quantity when backend add replaces existing quantity',
      () async {
        final repo = _MockCartRepository(
          serverItems: [_item(id: 'item-1', productId: 'prod-1', quantity: 1)],
          addReplacesExistingProductQuantity: true,
        );
        final container = _container(repo);
        addTearDown(container.dispose);

        await container.read(cartProvider.notifier).loadCart();
        await container.read(cartProvider.notifier).addItem('prod-1', 4);
        ...
        expect(repo.serverItems.single.quantity, 5);
        expect(item.quantity, 5);
      },
    );

    test(
      'addItem updates loaded cart item without hitting duplicate-item add path',
      () async {
        final repo = _MockCartRepository(
          serverItems: [_item(id: 'item-1', productId: 'prod-1', quantity: 2)],
          addResponses: [
    ...
    test(
      'addItem resolves CART_ITEM_ALREADY_EXISTS even when local cart is empty',
      () async {
        final repo = _MockCartRepository(
          serverItems: [_item(id: 'item-1', productId: 'prod-1', quantity: 2)],
          addResponses: [
    ...
    test(
      'addItem survives bootstrap race when USER_NOT_FOUND resolves after multiple retries',
      () async {
        final repo = _MockCartRepository(
          addResponses: [
